...
> 从招标文件到投标文件，你只需要说一句话，剩下的全部自动完成。

下面以一份真实的医院后勤综合服务招标文件为例，演示完整的使用过程。

### 你只需要做这一步

把招标文件交给 agent，告诉它页数要求：

> 📥 **输入**：[后勤综合服务招标文件.docx](example/后勤综合服务招标文件（N510681202600007020260603003）.docx)（公开招标文件）

> *"根据这份招标文件，使用 bid-assets-maker 制作投标文件，页数要求20页。"*

### agent 自动完成的工作

**1. 自动生成编制计划**

agent 会自动解析招标文件中的评分办法，按分值比例分配页数和字数，输出 [plan.md](example/output/plan.md)：

| 章节 | 分值 | 分配页数 | 内容摘要 |
|------|------|---------|---------|
| 全院环境清洁消毒服务方案 | 12分 | 3页 | 组织架构、设备配置、清洁消毒标准流程 |
| 中央运输服务方案 | 9分 | 2页 | 调度中心管理、标本运送、患者运送 |
| 维修维护服务方案 | 15分 | 3页 | 综合维修、锅炉系统、医气系统、污水处理 |
| 院感控制方案 | 12分 | 3页 | 医废运送院感、特殊科室管理、培训方案 |
| 人员配置方案 | 12分 | 3页 | 岗位人数、科室配置、服务频次、替代机制 |
| 管理体系和质量控制体系 | 4分 | 1页 | 管理体系、监管考核、满意度控制 |
| 应急预案 | 5分 | 1页 | 人员储备、疫情应急、自然灾害应急 |
| 一站式服务中心方案 | 9分 | 2页 | 建设方案、组织架构、运行方案 |
| 履约能力 | 12分 | 2页 | 类似项目业绩、采购人综合评价 |

**2. 确认或调整计划**

plan.md 会列出每个章节的评分细则和投标要点，你可以直接确认开始生成；**如果对计划不满意，告诉 agent 哪里需要调整，它会按你的要求修改后再开始生成。**

**3. 自动生成投标文件**

确认计划后，agent 逐章节生成图文并茂的内容（流程图、柱状图、饼图等），最终打包输出：

> 📤 **输出**：[后勤综合服务-投标文件.docx](example/output/后勤综合服务-投标文件.docx)（20页，含图表）

**案例生成的完整目录结构：**

```
example/output/
├── plan.md                                    # 编制计划（目录框架 + 页数分配 + 评分细则）
├── rfq-docs/                                  # 解析后的招标文件
│   ├── index.md
│   └── 全文.md
├── bid-docs/                                  # 根据评分项生成的投标素材（9个章节）
│   ├── 全院环境清洁消毒服务方案/
│   │   ├── 整体组织架构策划.md
│   │   ├── 设备物品配置.md
│   │   ├── 病区清洁消毒标准流程.md
│   │   ├── 公共区域维护消毒.md
│   │   └── assets/                            # 每节配图（流程图、清单图等）
│   ├── 中央运输服务方案/
│   │   └── assets/                            
│   ├── 维修维护服务方案/
│   ├── 院感控制方案/
...
│   │   └── assets/                            
│   └── 履约能力/
└── 后勤综合服务-投标文件.docx                  # 最终打包输出的投标文件 ✅
```

每个章节目录下都包含：按评分细则拆分的子章节 Markdown 文件、`assets/` 目录下的 HTML 图表源码及对应 PNG 截图、以及合并后的 `*-complete.md` 完整章节文件。

### 整个过程总结

```
你说一句话 → agent 自动规划 → 你确认/调整计划 → agent 自动生成 → 拿到投标文件
```

**两步操作，全程自动。** 无需手动分析评分细则、分配页数、组织内容——这些全部交给 agent 完成。
